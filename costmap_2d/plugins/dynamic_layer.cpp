...
PLUGINLIB_EXPORT_CLASS(costmap_2d::DynamicLayer, costmap_2d::Layer)
...
void DynamicLayer::onInitialize()
{
  ...
  if(subscribe_to_updates_)
  {
    ...
    map_update_sub_ = g_nh.subscribe(map_topic + "_updates", 10, &DynamicLayer::incomingUpdate, this);
  }
...
void DynamicLayer::incomingUpdate(const map_msgs::OccupancyGridUpdateConstPtr& update)
{
    unsigned int di = 0;
...
void DynamicLayer::activate()
{
    onInitialize();
}

void DynamicLayer::deactivate()
{
    map_sub_.shutdown();
    if (subscribe_to_updates_)
...
void DynamicLayer::updateBounds(double robot_x, double robot_y, double robot_yaw, double* min_x, double* min_y,
